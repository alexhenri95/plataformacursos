<section>
    <div class="card">
        <div class="card-body">
            <h1 class="font-semibold text-xl mb-2">Valoración:</h1>

            @can('enrolled', $course)
                <article class="mb-4">
                    @can('valued', $course)
                    <textarea wire:model.defer="comment" rows="3" class="form-input w-full rounded" placeholder="Ingrese una reseña del curso"></textarea>
                    <div class="flex items-center justify-between">
                        <button class="btn btn-primary btn-sm" wire:click="store">Publicar</button>
            ...
            <p class="text-blue-500 text-base mb-2 text-right font-bold">{{$course->reviews->count()}} valoraciones</p>
            @foreach($course->reviews as $review)
                <article class="flex mb-4 text-gray-800">
                    <figure class="mr-4">
                        <img src="{{$review->user->profile_photo_url}}" class="w-12 h-12 object-cover rounded-full object-center shadow" alt="">
                    </figure>
